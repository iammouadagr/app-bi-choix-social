...
def schulze(votes):
    """
        Algorithme de la méthode Schulze.
    ...
    num_candidates = len(set(itertools.chain(*votes)))
    pairwise_votes = np.zeros((num_candidates, num_candidates))
    for i in range(1, num_candidates):
        for j in range(1, num_candidates):
            if i != j:
                pairwise_votes[i][j] = sum([1 for k in range(1, num_candidates)
    ...
    sorted_candidates = sorted(range(1, num_candidates), key=lambda x: beatpath_strength[x], reverse=True)
